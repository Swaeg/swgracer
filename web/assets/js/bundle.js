...
var Game = function Game() {
    ...
    self.renderRoad = function () {
        ...
        var playerHeight = self.render.cameraHeight + playerPositionSegmentHeight + (playerNextPositionSegmentHeight - playerPositionSegmentHeight) * playerRelativePosition;
        var baseOffset = currentSegment.curve + (self.roadData[(currentSegmentIndex + 1) % self.roadData.length].curve - currentSegment.curve) * playerRelativePosition;
        ...
        while (j--) {
            ...
            var startProjectionHeight = Math.floor((playerHeight - currentSegment.height) * self.render.cameraDistance / (self.render.cameraDistance + currentSegmentPosition));
            var startScaling = 30 / (self.render.cameraDistance + currentSegmentPosition);

            var endProjectionHeight = Math.floor((playerHeight - nextSegment.height) * self.render.cameraDistance / (self.render.cameraDistance + currentSegmentPosition + self.segmentSize));
            ...
            if (currentHeight > endProjectionHeight) {
                self.drawSegment(halfHeight + currentHeight, currentScaling, currentSegment.curve - baseOffset - self.lastDelta * currentScaling, halfHeight + endProjectionHeight, endScaling, nextSegment.curve - baseOffset - self.lastDelta * endScaling, segmentColourCounter < self.segmentsPerColour, currentSegmentIndex == 2 || currentSegmentIndex == self.roadParameters.zones - self.render.depthOfField);
            }
            if (currentSegment.sprite) {
                spriteBuffer.push({
                    x: halfWidth - currentSegment.sprite.pos * self.render.width * currentScaling + currentSegment.curve - baseOffset - (self.player.posX - baseOffset * 2) * currentScaling,
